...
## Логирование
- Фоновые службы используют встроенный `ILogger` и стандартный вывод хоста.  
- Сообщения об ошибках и предупреждениях выводятся в stdout/stderr, что упрощает интеграцию с `systemd` и `journalctl`.  
- Для детальной диагностики можно расширить регистрацию логгеров в `Program` (например, подключить Serilog или централизованные сервисы).
